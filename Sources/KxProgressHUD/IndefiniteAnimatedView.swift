...
extension IndefiniteAnimatedView {
  ...
  private func loadImageBundle(named imageName: String) -> UIImage? {
    let bundle = Bundle.init(for: IndefiniteAnimatedView.self)
    guard let bundlePath = bundle.path(forResource: "KxProgressHUD_KxProgressHUD", ofType: "bundle") ?? bundle.path(forResource: "KxProgressHUD", ofType: "bundle")  else { return nil }
