...
open class KxProgressHUD : UIView {
  ...
  private func updateHUDFrame() {
    ...
    if let text = getStatusLabel().text {
      ...
      let parentWidth: CGFloat = bounds.width == 0 ? window?.bounds.width ?? bounds.width : bounds.width
      let constraintSize = CGSize(width: min(500, parentWidth - KxProgressHUDHorizontalSpacing * 2), height: 200.0) // Increased width constraint
    ...
    if getStatusLabel().text != nil && (imageUsed || progressUsed) {
      ...
      hudHeight += CGFloat(KxProgressHUDLabelSpacing)
    }
    
    // Ensure minimum width and height constraints are respected
    hudWidth = max(minimumSize.width, hudWidth)
    hudHeight = max(minimumSize.height, hudHeight)
    ...
    var centerY: CGFloat
    if getStatusLabel().text != nil {
      let yOffset = max(KxProgressHUDVerticalSpacing, (minimumSize.height - contentHeight - CGFloat(KxProgressHUDLabelSpacing) - labelHeight) / 2.0)
    ...
    // Update the label frame to use the adjusted width
    getStatusLabel().frame = CGRect(x: 0, y: 0, width: labelWidth, height: labelHeight)
